...
def generate_html(data: dict, output_path: str):
    ...
    if opportunities:
        ...
        if tam:
            tam_html = f"""
            <div style="margin:12px 0 4px 0;font-size:11px;color:#64748B;line-height:1.5">
              <strong>TAM</strong> = Total Addressable Market &nbsp;|&nbsp;
              <strong>SAM</strong> = Serviceable Addressable Market &nbsp;|&nbsp;
              <strong>SOM</strong> = Serviceable Obtainable Market
            </div>
            <div style="display:flex;gap:12px;margin:4px 0 12px 0;flex-wrap:wrap">
              <div style="flex:1;min-width:120px;background:#F0FDFA;padding:12px 10px;border-radius:6px;text-align:center">
                <div style="font-size:10px;color:{TEAL_DARK};text-transform:uppercase;letter-spacing:0.5px;font-weight:600">TAM</div>
                <div style="font-size:9px;color:#64748B;margin:2px 0 4px 0">Total Addressable Market</div>
                <div style="font-size:16px;font-weight:700;color:{TEAL_DARK}">{escape_html(tam.get('total_addressable', 'N/A'))}</div>
              </div>
              <div style="flex:1;min-width:120px;background:#F0FDFA;padding:12px 10px;border-radius:6px;text-align:center">
                <div style="font-size:10px;color:{TEAL_DARK};text-transform:uppercase;letter-spacing:0.5px;font-weight:600">SAM</div>
                <div style="font-size:9px;color:#64748B;margin:2px 0 4px 0">Serviceable Addressable Market</div>
                <div style="font-size:16px;font-weight:700;color:{TEAL_DARK}">{escape_html(tam.get('serviceable_addressable', 'N/A'))}</div>
              </div>
              <div style="flex:1;min-width:120px;background:#F0FDFA;padding:12px 10px;border-radius:6px;text-align:center">
                <div style="font-size:10px;color:{TEAL_DARK};text-transform:uppercase;letter-spacing:0.5px;font-weight:600">SOM Y1</div>
                <div style="font-size:9px;color:#64748B;margin:2px 0 4px 0">Obtainable Market (Year 1)</div>
                <div style="font-size:16px;font-weight:700;color:{TEAL_DARK}">{escape_html(tam.get('obtainable_y1', 'N/A'))}</div>
              </div>
              <div style="flex:1;min-width:120px;background:#F0FDFA;padding:12px 10px;border-radius:6px;text-align:center">
                <div style="font-size:10px;color:{TEAL_DARK};text-transform:uppercase;letter-spacing:0.5px;font-weight:600">SOM Y3</div>
                <div style="font-size:9px;color:#64748B;margin:2px 0 4px 0">Obtainable Market (Year 3)</div>
    ...
    if fm and fm.get("features"):
        ...
        for i, feat in enumerate(features):
            ...
            for comp_name, vals in competitors.items():
                ...
                if val:
                    cells += f"<td style='text-align:center'><span style='color:#059669;font-size:16px;font-weight:bold'>&#10003;</span></td>"
                else:
                    cells += f"<td style='text-align:center'><span style='color:#DC2626;font-size:14px'>&#10007;</span></td>"
...
def _pdf_check_page_break(pdf, min_space_mm=35):
    """Add page break if less than min_space_mm remains on current page."""
...
def _pdf_tam_cards(pdf, tam):
    """Draw TAM/SAM/SOM as side-by-side cards with full definitions."""
    TAM_DEFINITIONS = {
        "TAM": "Total Addressable Market",
        "SAM": "Serviceable Addressable Market",
        "SOM Y1": "Serviceable Obtainable Market (Year 1)",
        "SOM Y3": "Serviceable Obtainable Market (Year 3)",
    }
    ...
    card_h = 22  # Increased from 14mm for spacing
    ...
    for i, (label, key) in enumerate(labels):
        ...
        pdf.cell(card_w, 3, TAM_DEFINITIONS.get(label, ""), align="C")
        # Value - auto-size font for long values
        val = latin_safe(tam.get(key, "N/A"))
        font_size = 10
        pdf.set_font("Helvetica", "B", font_size)
        while pdf.get_string_width(val) > card_w - 4 and font_size > 6:
            font_size -= 0.5
            pdf.set_font("Helvetica", "B", font_size)
        pdf.set_xy(x, y + 11)
...
def generate_pdf(data: dict, output_path: str):
    ...
    if opportunities:
        ...
        usable_w = pdf.w - pdf.l_margin - pdf.r_margin - 2  # 2mm safety margin
        label_w = 44
        score_w = 16
        wt_w = 20
        bar_w = usable_w - label_w - score_w - wt_w  # fills remaining space
        ...
        for idx, dim in enumerate(top.get("dimensions", [])):
            ...
            # Restore row fill color after score bar changed it
            if idx % 2 == 0:
                pdf.set_fill_color(248, 250, 252)
            else:
                pdf.set_fill_color(255, 255, 255)
        ...
        tam = top.get("tam", {})
        if tam:
            _pdf_check_page_break(pdf, 40)
            pdf.set_text_color(*TEAL_RGB)
            pdf.set_font("Helvetica", "B", 10)
            pdf.cell(0, 6, "Market Sizing", new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("Helvetica", "", 7)
            pdf.set_text_color(100, 116, 139)
            pdf.cell(0, 4, "TAM = Total Addressable Market  |  SAM = Serviceable Addressable Market  |  SOM = Serviceable Obtainable Market", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(2)
    ...
    if competitors:
        ...
        c_widths = [int(usable * 0.18), int(usable * 0.14), int(usable * 0.34), int(usable * 0.34)]
        ...
        for idx, comp in enumerate(competitors):
            ...
            name = latin_safe(comp.get("name", ""))
            pricing = latin_safe(comp.get("pricing", ""))
            strengths = latin_safe(", ".join(comp.get("strengths", [])))
            weaknesses = latin_safe(", ".join(comp.get("weaknesses", [])))
            # Calculate row height based on longest text
            texts = [name, pricing, strengths, weaknesses]
            max_lines = 1
            for i, txt in enumerate(texts):
                n_lines = max(1, len(txt) // max(1, int(c_widths[i] / 1.8)) + 1)
                max_lines = max(max_lines, n_lines)
            row_h = max(5.5, max_lines * line_h)
            # Check if row fits on current page, add page break + re-draw header if not
            remaining = pdf.h - pdf.get_y() - pdf.b_margin
            if remaining < row_h + 2:
                pdf.add_page()
                _pdf_table_header(pdf, c_headers, c_widths)
                pdf.set_font("Helvetica", "", 7)
            # Disable auto page break to prevent mid-row splits
            ...
            for w in c_widths:
                ...
                pdf.rect(x, row_y, w, row_h, style="DF")
                x += w
            # Write text into each cell
            pdf.set_text_color(60, 60, 60)
            pdf.set_xy(pdf.l_margin + 1, row_y + 0.5)
            pdf.set_font("Helvetica", "B", 7)
            pdf.multi_cell(c_widths[0] - 2, line_h, name)
            pdf.set_xy(pdf.l_margin + c_widths[0] + 1, row_y + 0.5)
            pdf.set_font("Helvetica", "", 7)
            pdf.multi_cell(c_widths[1] - 2, line_h, pricing)
            pdf.set_xy(pdf.l_margin + c_widths[0] + c_widths[1] + 1, row_y + 0.5)
            pdf.multi_cell(c_widths[2] - 2, line_h, strengths)
            pdf.set_xy(pdf.l_margin + c_widths[0] + c_widths[1] + c_widths[2] + 1, row_y + 0.5)
            pdf.multi_cell(c_widths[3] - 2, line_h, weaknesses)
    ...
    if risk_register := data.get("risk_register", []):
        ...
        usable = pdf.w - pdf.l_margin - pdf.r_margin
        r_widths = [18, 22, 18, usable - 18 - 22 - 18]
        ...
        for idx, risk in enumerate(risk_register):
            bg = (248, 250, 252) if idx % 2 == 0 else (255, 255, 255)
            desc = latin_safe(risk.get("description", ""))
            # Calculate row height from description length
            desc_chars_per_line = max(1, int(r_widths[3] / 1.8))
            n_lines = max(1, len(desc) // desc_chars_per_line + 1)
            row_h = max(5.5, n_lines * line_h)
            # Check if row fits on current page, add page break + re-draw header if not
            remaining = pdf.h - pdf.get_y() - pdf.b_margin
            if remaining < row_h + 2:
                pdf.add_page()
            ...
            pdf.set_auto_page_break(auto=False)
            row_y = pdf.get_y()
            # Draw background rects for all cells
            x = pdf.l_margin
            for w in r_widths:
                pdf.set_fill_color(*bg)
                pdf.rect(x, row_y, w, row_h, style="DF")
            ...
            pdf.set_font("Helvetica", "B", 7)
            pdf.set_text_color(60, 60, 60)
            pdf.cell(r_widths[0] - 2, line_h, latin_safe(risk.get("id", "")))
            # Category
            pdf.set_xy(pdf.l_margin + r_widths[0] + 1, row_y + 0.5)
            pdf.set_font("Helvetica", "", 7)
            pdf.cell(r_widths[1] - 2, line_h, latin_safe(risk.get("category", "")))
            # Severity with color
            sev = risk.get("severity", "medium").lower()
            sev_colors = {
            ...
            sr, sg, sb = sev_colors.get(sev, (133, 77, 14))
            pdf.set_xy(pdf.l_margin + r_widths[0] + r_widths[1] + 1, row_y + 0.5)
            pdf.set_font("Helvetica", "B", 7)
            pdf.set_text_color(sr, sg, sb)
            pdf.cell(r_widths[2] - 2, line_h, sev.upper(), align="C")
            # Description - full text with multi_cell
            pdf.set_xy(pdf.l_margin + r_widths[0] + r_widths[1] + r_widths[2] + 1, row_y + 0.5)
            pdf.set_font("Helvetica", "", 7)
            pdf.set_text_color(60, 60, 60)
            pdf.multi_cell(r_widths[3] - 2, line_h, desc)
            pdf.set_y(row_y + row_h)
            # Re-enable auto page break
            pdf.set_auto_page_break(auto=True, margin=20)
        pdf.ln(4)
...
def generate_md(data: dict, output_path: str):
    ...
    if opportunities:
        ...
        if tam:
            ...
            lines.append("")
            lines.append("> **TAM** = Total Addressable Market | **SAM** = Serviceable Addressable Market | **SOM** = Serviceable Obtainable Market")
            lines.append("")
            lines.append(f"- **TAM** (Total Addressable Market): {tam.get('total_addressable', 'N/A')}")
            lines.append(f"- **SAM** (Serviceable Addressable Market): {tam.get('serviceable_addressable', 'N/A')}")
            lines.append(f"- **SOM Year 1** (Serviceable Obtainable Market): {tam.get('obtainable_y1', 'N/A')}")
            lines.append(f"- **SOM Year 3** (Serviceable Obtainable Market): {tam.get('obtainable_y3', 'N/A')}")
            lines.append("")
    ...
    # Demand Signals
    demand_signals = market.get("demand_signals", [])
    if demand_signals:
        lines.append("## Demand Signals")
        lines.append("")
        lines.append("| Signal | Strength | Source | Confidence |")
        lines.append("|--------|----------|--------|------------|")
        for ds in demand_signals:
            lines.append(f"| {ds.get('signal', '')} | {ds.get('strength', '')} | {ds.get('source', '')} | {ds.get('confidence', '')} |")
        lines.append("")
